...
export const signup = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { name, email, telefone, password, confirmPassword, status, role } = req.body
    if(password !== confirmPassword){
      next(new BadRequestException("As senhas não conferem", ErrorCode.BAD_REQUEST))
    }
...
export const login = async (req: Request, res: Response) => {
  ...
  if (!user) {
    throw new UserNotFoundException('Usuário não encontrado', ErrorCode.USER_NOT_FOUND)
  }
